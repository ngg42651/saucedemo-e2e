...
| 세션 | 1 | 로그아웃 후 세션 무효화 확인(직접 접근 차단) |
| **합계** | **20** | |
...
- **미로그인 직접 접근 차단** (`LoginTests.로그인하지_않고_상품_목록에_직접_접근하면_차단된다`):
  ...
  라우팅을 놓칠 수 있다.
- **로그아웃 후 직접 접근 차단** (`SessionTests.로그아웃하면_세션이_무효화되어_직접_접근이_차단된다`):
  바로 위 `LoginTests` 케이스와 겹쳐 보이지만 검증 대상이 다르다. 위는 **세션을
  만든 적이 없는 상태**, 이쪽은 **세션을 만들었다가 지운 상태**다. 로그아웃
  버튼이 화면만 바꾸고 서버 세션을 무효화하지 않으면 앞 케이스는 통과하고 이
  케이스만 실패한다. 실제 사고가 나는 쪽은 후자라 별도 축으로 두었다.
...
- 배송정보 필수 입력 오류 3종 중 2종(Last Name, Postal Code): 이 사이트는 빈
  칸을 위에서부터 하나씩 검사해 **First Name 오류를 띄운 시점에 나머지는 도달할
  수 없다**. 세 케이스를 만들려면 앞 칸을 채워가며 3번 돌려야 하는데, 검증되는
  것은 같은 필수값 검사 로직 하나다. 실행 시간만 3배가 되어 첫 칸만 남겼다.
...
이 저장소는 **Claude Code로 작성했다.** `.claude/agents/`와 `docs/superpowers/`가 함께
공개돼 있는 것은 지우지 않은 게 아니라 남겨 둔 것이다 — 결과 코드만 남기면 **무엇을 왜
그렇게 정했는지가 사라진다.**

작업은 셋으로 나눴다.

1. **설계 문서** (`docs/superpowers/specs/`) — 목적, 대상 사이트 선정 이유, 케이스 범위를
   먼저 확정하고 승인한 뒤에 코드를 시작했다.
2. **구현 계획** (`docs/superpowers/plans/`) — Task 단위로 쪼개고 아키텍처 결정(POM 경계,
   `PageTest` 기반 테스트 격리, 스택)을 명시했다.
3. **리뷰 에이전트** (`.claude/agents/reviewer.md`) — 구현된 diff를 `CLAUDE.md`의 불변 규칙
   기준으로 검사한다.

**리뷰 에이전트의 1순위 점검 항목이 "assert 약화"다.** 이전 커밋보다 검증이 느슨해졌는지 —
삭제된 assert, 범위가 넓어진 비교, 사라진 조건 — 를 가장 먼저 찾는다. 칭찬을 금지하고
요청 범위 밖의 개선 제안도 막아 뒀다. `CLAUDE.md` 규칙 4번("통과시키려고 assert를
약화시키지 않는다")을 사람이 매번 기억하는 대신 **검사로 만든 것**이다.

사람이 정한 것과 도구가 한 일을 구분하면 이렇다.

| | 내용 |
|---|---|
| **사람이 정함** | 케이스 선정과 제외 기준 · 불변 규칙 6개 · 총액을 화면 값끼리 비교하지 않고 독립 계산한다는 판단 · `problem_user` 검증에 대조군을 두는 설계 · 스케줄 실행과 재시도를 붙이지 않는다는 결정 |
| **도구가 함** | 위 결정을 코드로 옮기는 작업 · 규칙 위반 기계 검사 |

문서로 남은 결정이 코드보다 오래 간다고 보고 설계·계획·리뷰 기준을 전부 저장소에 뒀다.
